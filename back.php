...
//	$link = mysqli_connect("localhost", "user", "password", "database"); // db connect
    ...
    if (!empty($_POST["wallet"]))
    {
	    ...
	    if ($isValid) {
			//do upon successful signature verification, for example:
//			$query = mysqli_query($link, "UPDATE `table` SET `ton-wallet`='".$_POST["wallet"]."' WHERE `link`='".$_POST["proof"]["payload"]."'"); //recording a user-friendly wallet in the database
	    } else {
	        //do if signature verification fails
	    }
    }
